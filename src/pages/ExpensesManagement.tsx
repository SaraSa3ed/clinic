...
export default function ExpensesManagement() {
  ...
  const [updateExpenseStatus, { isLoading: statusLoading }] = useUpdateExpenseStatusMutation();
  ...
  const handleAddExpense = async () => {
    ...
    try {
      await createExpense({
        ...
        vendorContact: newExpense.vendorContact,
  ...
  // تحديث حالة المصروف
  const handleUpdateStatus = async (id: number, status: 'approved' | 'rejected', rejectionReason?: string) => {
    try {
  ...
  const handleExportExpenses = async () => {
    try {
      ...
      const csvData = result.data.expenses.map((expense: any) => ({
        'العنوان': expense.العنوان,
        'المبلغ': expense.المبلغ,
        'الفئة': expense.الفئة,
        'التاريخ': expense.التاريخ,
        'الحالة': expense.الحالة,
        'رقم الإيصال': expense['رقم الإيصال'],
        'طريقة الدفع': expense['طريقة الدفع'],
        'اسم المورد': expense['اسم المورد'],
        'الوصف': expense.الوصف
      }));
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">إدارة وتتبع جميع المصييسسيروفات والمصاريف</p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{statistics.rejectedCount}</p>
              </div>
              <div className="p-3 bg-red-100 rounded-full">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{selectedExpense.vendorName || '-'}</p>
                </div>
              </div>
              {selectedExpense.receiptNumber && (
                <div>
                  <Label className="text-sm font-medium text-gray-600">رقم الإيصال</Label>
                  <p className="text-gray-900">{selectedExpense.receiptNumber}</p>
                </div>
              )}
              {selectedExpense.rejectionReason && (
                <div>
                  <Label className="text-sm font-medium text-gray-600">سبب الرفض</Label>
                  <p className="text-gray-900">{selectedExpense.rejectionReason}</p>
                </div>
              )}
            </div>
            <DialogFooter>
